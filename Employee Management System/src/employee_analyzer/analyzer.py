...
def analyze_experience(employees):
    if not employees:
        return {"average": 0,"highest": None,"lowest": None}
    ...
    return {"average": average_experience,"highest": highest_experience,"lowest": lowest_experience}
...
def search_employees(employees, search_text):
    ...
    for employee in employees:
        if (search_text in employee["id"].lower() or search_text in employee["name"].lower()):
