...
def load_data():
    ...
    try:
        ...
        if not isinstance(data, list):
            logger.error(
                "Invalid data format. JSON data is not a list."
            )
            print(
                "Invalid data format. Starting with empty employee list."
            )
            return
        ...
        for employee in data:
            if not is_valid_employee(employee):
                logger.error(
                    "Invalid employee data found in JSON file: %s",
                    employee
                )
        ...
        logger.info(
            "Employee data loaded successfully. Total employees: %s",
            len(employees)
        )
        ...
        logger.error(
            "Could not read the data file: %s",
            error
        )
...
def save_data():
    try:
        ...
        logger.info(
            "Employee data saved successfully. Total employees: %s",
            len(employees)
        )
        ...
        logger.error(
            "Could not save the data file: %s",
            error
        )
...
def handle_menu_choice(choice):
    if choice == 1:
        ...
        save_data()
        logger.info(
            "Employee Management System exited safely."
        )
...
def main():
    ...
    try:
        ...
        logger.warning(
            "Program interrupted by user using Ctrl+C."
        )
